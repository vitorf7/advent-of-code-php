...
class ElfGuideSpec extends ObjectBehavior
{
    ...
    public function it_should_be_able_to_have_many_santas_passed_in(Santa $santa1, Santa $roboSanta)
    {
        $this->beConstructedWith([$santa1, $roboSanta]);

        $this->getSantas()->shouldBeArray();
    }

    public function it_should_give_directions_to_each_of_two_santas_alternatively(Santa $santa1, Santa $roboSanta)
    {
        $this->beConstructedWith([$santa1, $roboSanta]);

        $this->readDirections('^v');

        $santa1->moveNorth()->shouldBeCalledTimes(1);
        $roboSanta->moveSouth()->shouldBeCalledTimes(1);
    }

    public function it_should_give_directions_to_each_santa_alternatively(Santa $santa1, Santa $roboSanta)
    {
        $this->beConstructedWith([$santa1, $roboSanta]);
        ...
        $santa1->moveNorth()->shouldBeCalledTimes(1);
        $roboSanta->moveSouth()->shouldBeCalledTimes(1);
        $santa1->moveWest()->shouldBeCalledTimes(1);
        $roboSanta->moveEast()->shouldBeCalledTimes(1);
    }

    public function it_should_be_able_to_tell_3_santas_their_directions_alternatively(Santa $santa1, Santa $santa2, Santa $santa3)
    {
        $this->beConstructedWith([$santa1, $santa2, $santa3]);

        $this->readDirections('^v<');

        $santa1->moveNorth()->shouldBeCalledTimes(1);
        $santa2->moveSouth()->shouldBeCalledTimes(1);
        $santa3->moveWest()->shouldBeCalledTimes(1);
    }
}
